  ...
  return (
    <Surface padding="none">
      <Link
        ...
        className="flex items-center gap-4 rounded-[inherit] p-5 transition-colors hover:bg-hover motion-reduce:transition-none sm:p-6"
      >
        <span className="shrink-0 text-[13px] font-medium text-fg-muted">Team</span>
        <ul className="flex min-w-0 flex-1 items-center gap-3 overflow-x-auto" aria-hidden>
          {team.map((m) => (
            <li key={m.userId} className="relative shrink-0">
              <span
                className={cn(
                  "inline-grid h-11 w-11 place-items-center rounded-full border text-[13px] font-semibold",
